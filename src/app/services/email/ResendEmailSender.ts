...
import {
  IEmailSender,
  SendEmailVerificationEmailData,
...
export class ResendEmailSender implements IEmailSender {
  ...
  async sendEmailVerificationEmail(
    ...
    const result = await this.resend.emails.send({
      from: env.resendFromEmail,
      to: data.to,
      subject: 'Confirmá tu email de rituo',
      html: this.emailVerificationHtml(data.verifyUrl),
      text: this.emailVerificationText(data.verifyUrl),
    });

    if (result.error) {
      this.logger.error({
        event: 'email_verification_email_failed',
        to: data.to,
        error: result.error,
      });
    ...
    return `
      ...
      <html lang="es">
        <head>
          ...
          <meta name="color-scheme" content="light" />
          <meta name="supported-color-schemes" content="light" />
          <title>Confirmá tu email de rituo</title>
        </head>
        <body style="margin:0;padding:0;background:#eef3fb;color:#111827;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,Arial,sans-serif;">
          <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="width:100%;background:#eef3fb;margin:0;padding:0;">
            <tr>
                ...
                <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="width:100%;max-width:560px;border-collapse:separate;border-spacing:0;">
                  ...
                  <tr>
                    <td style="border:1px solid #d9e2ef;border-radius:28px;background:#ffffff;box-shadow:0 18px 54px rgba(13,21,40,0.12);padding:32px 28px;">
                      ...
                      <p style="margin:16px 0 0;color:#4b5565;font-size:16px;line-height:1.62;">
                        Para terminar de activar tu cuenta, confirmá que este correo te pertenece.
                      </p>
                      ...
                      <table role="presentation" cellspacing="0" cellpadding="0" style="margin:26px 0 22px;">
                        <tr>
                          <td>
                            <a href="${verifyUrl}" style="display:inline-block;background:#111a3a;color:#ffffff;padding:15px 24px;border-radius:999px;text-decoration:none;font-size:15px;font-weight:850;box-shadow:0 12px 28px rgba(17,26,58,0.22);">
                              Confirmar email
                            </a>
                          </td>
                        </tr>
  ...
  private emailVerificationText(verifyUrl: string): string {
    return [
      'Confirmá tu email de rituo',
      '',
      'Para terminar de activar tu cuenta, abrí este link:',
      verifyUrl,
      '',
      'Si no creaste esta cuenta, podés ignorar este email.',
    ].join('\n');
